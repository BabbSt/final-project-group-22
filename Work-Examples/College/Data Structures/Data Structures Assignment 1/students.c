...
struct student* deep_copy_student(struct student* student) {
	...
	s->name=malloc((1+strlen(student->name))*sizeof(char));
	s->id=student->id;
	s->gpa=student->gpa;
...
void print_students(struct student* students, int num_students) {
	for(int i=0;i<num_students;i++){
		printf("Student %d is named %s, has the id %d and their GPA is %f\n", i, students[i].name, students[i].id,students[i].gpa);
	//	printf("Student %d is named , has the id %d and their GPA is %f\n", i, students[i].id,students[i].gpa);
	}
}
...
struct student* find_min_gpa(struct student* students, int num_students) {
	...
	float m=students[0].gpa;
	for(int i=1; i<num_students; i++){
		//printf("befor: m=%f  s=%s",m,s->name);
		if(students[i].gpa<m){
			m=students[i].gpa;
			s=students+i;
		}
		//printf("after: m=%f  s=%s",m,s->name);
	}
  return s;
}
...
void sort_by_gpa(struct student* students, int num_students) {
	...
	for(int i=num_students; i>0; i--){
		m=deep_copy_student(find_min_gpa(students,i));
		x=0;
		while(students[x].id!=m->id){
			x++;
		}
		for(int j=x;j<i-1;j++){
