...
int bst_path_sum(int sum, struct bst* bst) {
	...
	if(temp->left!=NULL){
		bst->root=temp->left;
		ls+=bst_path_sum(sum-temp->val,bst);
	}
	if(temp->right!=NULL){
		bst->root=temp->right;
		rs+=bst_path_sum(sum-temp->val,bst);
	}
	if(temp->right==NULL&&temp->left==NULL){
		if(sum==temp->val){
			return 1;
...
void tree_to_stack(struct bst* bst, struct bst_iterator* bi){
   	struct bst_node* temp=bst->root;
	if(temp->right!=NULL){
	   	bst->root=temp->right;
		tree_to_stack(bst,bi);
	}
	if(temp->right!=NULL||temp->left!=NULL){
		stack_push(bi->s,temp);
	}
	if(temp->left!=NULL){
		bst->root=temp->left;
		tree_to_stack(bst,bi);
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
	assert(bst);
	struct bst_iterator* bi=malloc(sizeof(struct bst_iterator));
	bi->s=stack_create();
	tree_to_stack(bst,bi);
  return bi;
}
...
int bst_iterator_next(struct bst_iterator* iter) {
	assert(iter->s);
	struct bst_node* t=stack_pop(iter->s);
   return t->val;
}
